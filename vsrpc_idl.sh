...
usage()
{
  ...
  echo "  --fn-prefix preffix      common C funcion prefix (like myrpc_)"
  echo
  echo "extra options:"
...
then
  ...
  do
    ...
    then # clean server autogenerated code
     ...
     #rm -f "${server_out_dir}/${prj}${common_c}" don't remove!
    ...
    then # clean client autogenerated code
     ...
     #rm -f "${client_out_dir}/${prj}${client_h}" don't remove!
...
common_proto="${RetType} ${ProcNameC}(vsrpc_t*${ArgProto});"

# pattern of common function (add to ${prj}${common_c} file)
common_func=`cat <<EOF
${RetType} ${ProcNameC}(vsrpc_t* rpc${ArgDesc})
{
...
${ArgDesc2}
  ...
  // call procedure
${ArgFill}  ${Ret2}${ProcNameC}(rpc${ArgAll});
